...
# Define Docker volume path for model cache
DOCKER_VOLUME_PATH = "/root/.cache/huggingface"
MODEL_NAME = "deepseek-ai/Janus-Pro-1B"
MODEL_CACHE_PATH = os.path.join(DOCKER_VOLUME_PATH, "models")
...
def load_model_and_processor():
    print(f"Loading model from Docker volume: {DOCKER_VOLUME_PATH}")

    # Set environment variables to force HuggingFace to use our cache directory
    os.environ["TRANSFORMERS_CACHE"] = MODEL_CACHE_PATH
    os.environ["HF_HOME"] = DOCKER_VOLUME_PATH

    try:
        print("Attempting to load from Docker volume cache...")

        # Load config
        config = AutoConfig.from_pretrained(
            MODEL_NAME, local_files_only=True, cache_dir=MODEL_CACHE_PATH
        )
        ...
        print("Successfully loaded all components from Docker volume cache")

    except Exception as e:
        print(f"Cache load failed, downloading components: {str(e)}")

        # Download and cache components
        config = AutoConfig.from_pretrained(MODEL_NAME, cache_dir=MODEL_CACHE_PATH)
        language_config = config.language_config
        language_config._attn_implementation = "eager"
        ...
        print("Downloaded and cached all components to Docker volume")
    ...
    if torch.cuda.is_available():
        ...
        print("Model using CPU (no GPU available)")
...
print(f"Initialization complete. Using device: {cuda_device}")
